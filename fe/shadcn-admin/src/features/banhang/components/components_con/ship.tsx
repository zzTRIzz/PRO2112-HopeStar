...
interface ShipProps {
  ...
  weight?: number
  address?: string
  onShippingFeeChange?: (shippingFee: number, insuranceFee: number) => void
}
...
const Ship = ({
  ...
  /> */}
      ...
      <div className='flex justify-between border-b pb-2'>
        ...
        >
          <p className='text-gray-700 text-base'>Thêm phí bảo hiểm</p>

        </Checkbox>
        {isInsuranceChecked ? (
          <span className="font-semibold">{new Intl.NumberFormat('vi-VN').format(insuranceFee)}đ</span>
        ) : (
          ''
        )}
      </div>
