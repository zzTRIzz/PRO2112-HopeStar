...
function BanHangTaiQuay() {
  ...
  const [dialogContent, setDialogContent] = useState<'product' | 'imei'>(
    'product'
  )
  ...
  const [ListVoucherTheoAccount, setListVoucherTheoAccount] = useState<
    Voucher[]
  >([])
  ...
  const handleThanhToan = async (status: string, billType: number) => {
    try {
      if (searchBill == null || searchBill?.id === undefined) {
        ...
        return
      } else if (paymentMethod == null && isThanhToanNhanHang == false) {
        ...
        if (result) {
          await thanhToan({
            ...
            customerAppointment_date:
              searchBill?.customerAppointment_date ?? null,
          ...
          fromThatBai(
            `Thanh toán đơn hàng ${searchBill?.nameBill ?? ''} không thành công`
          )
        }
      }
    } catch (error) {
