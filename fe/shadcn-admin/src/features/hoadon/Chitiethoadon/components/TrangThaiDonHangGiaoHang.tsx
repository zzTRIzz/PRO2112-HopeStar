...
const OrderStepper: React.FC<OrderStepperProps> = ({
  ...
  return (
    <div className='h-[160px] w-full overflow-x-auto pt-[15px]'>
      <div className='flex min-w-max gap-16 px-2'>
        {historySteps.map((item, index) => {
          const statusInfo = statusMap[item.actionType] || {
            title: 'Không rõ trạng thái',
            icon: <Icon icon='lucide:alert-circle' className='h-4 w-4' />,
            color: 'bg-gray-500',
          }
          ...
          return (
            <Step
              ...
              description={new Date(item.actionTime).toLocaleDateString(
                'vi-VN',
                {
                  hour: '2-digit',
                  minute: '2-digit',
  ...
  return (
    <div className={cn('relative flex flex-col items-center')}>
      ...
      <div className='relative z-10'>
        <div
          className={cn(
            'flex h-[65px] w-[90px] items-center justify-center rounded-full',
        ...
        >
          ...
          <div
            className={cn(
              'flex h-[57px] w-[82px] items-center justify-center rounded-full',
      ...
      <div className='mt-3 max-w-[160px] text-center'>
        <h3
          className={cn('text-sm font-semibold transition-colors', color_text)}
        >
          {title}
        </h3>
        <p className={cn('mt-1 text-xs font-medium', color_text)}>
          {description}
        </p>
      </div>
    </div>
...
const TrangThaiDonHangGiaoHang: React.FC<TrangThaiDonHangProps> = ({
  ...
  const invoiceData = {
    ...
    items:
      searchBill?.billDetailResponesList.map((detail) => ({
        product:
          detail.productDetail.productName +
          ' ' +
          detail.productDetail.ram +
          '/' +
          detail.productDetail.rom +
          detail.productDetail.descriptionRom +
          '( ' +
          detail.productDetail.color +
          ' )',
        imei: detail.imeiSoldRespones.map(
          (imeiSold) => imeiSold.id_Imei.imeiCode
        ),
        price: detail.price,
  ...
  return (
    <div className='w-full'>
      <div className='rounded-xl bg-white p-4 shadow-xl ring-1 ring-gray-200'>
        {/* Header */}
        <div className='ml-[10px] border-b border-gray-100 p-2'>
          <div className='flex items-center justify-between'>
            <h1 className='text-xl font-bold text-gray-800'>
              Chi tiết đơn hàng
            </h1>
            <span className='text-sm font-medium text-black'>
              Mã đơn hàng: {searchBill != null ? searchBill.maBill : ''}
            </span>
          </div>
        </div>
        ...
        <div className='w-full p-4'>
          ...
          <div className='mt-8 flex justify-center gap-2'>
            <Button
              ...
              className={cn(
                'rounded-md px-4 py-2 text-white transition-all duration-300',
                'ml-[10px] flex items-center gap-2 bg-orange-500 hover:bg-orange-600'
              )}
            >
              {getNextStatus()
                ? `${statusMap[getNextStatus()!]?.title}`
                : 'Xác nhận'}
            </Button>
            {currentStatus === 'DANG_GIAO_HANG' && (
              <Button
                ...
                className={cn(
                  'rounded-md px-2 py-2 text-white transition-all duration-300',
